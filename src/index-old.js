...
document.addEventListener(
  'DOMContentLoaded', function(){
    // const screencast = navigator.mediaDevices.getDisplayMedia(
    //   {
    //     video: {
    //         cursor: 'always',
    ...
    const deviceStream = document.querySelector('#deviceStream');

    const screencast = navigator.mediaDevices.getUserMedia(
      {
        audio: false,
    ...
    screencast.then(
      stream => {
        deviceStream.srcObject = stream;
        // console.dir(deviceStreamUrl);
        // deviceStream.src = deviceStreamUrl.createObjectURL(stream);
        // deviceStream.play();
        // deviceStream.height = stream.getVideoTracks()[0].getSettings().width;
        // deviceStream.width = stream.getVideoTracks()[0].getSettings().height;
        // console.dir(deviceStream.videoHeight);
      }
    );
    ...
    snapshot.addEventListener(
      'click', () => {
        ...
        context.drawImage(deviceStream, 0, 0, width, height);
      }
    );

    deviceStream.addEventListener(
      'play', (e) => {
        draw(e.target, context, width, height);
        setInterval( ()=> {draw(e.target, context, width, height);}, 100);
      }, false
    );
